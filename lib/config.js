...
module.exports = {
  ...
  keys: {
    ...
    // Google Earth Engine service account. The private key may arrive with literal "\n"
    // sequences (single-quoted .env value, CI secret) - normalized to real newlines here.
    earthEngine: process.env.EARTH_ENGINE_PROJECT && process.env.EARTH_ENGINE_SERVICE_ACCOUNT && process.env.EARTH_ENGINE_PRIVATE_KEY
      ? {
          project: process.env.EARTH_ENGINE_PROJECT,
          clientEmail: process.env.EARTH_ENGINE_SERVICE_ACCOUNT,
          privateKey: process.env.EARTH_ENGINE_PRIVATE_KEY.replace(/\\n/g, '\n'),
        }
      : null,
    // Not read by any live adapter yet (both connectors are planned stubs) - centralized
    // here anyway, so /api/sources can honestly show keyConfigured for them.
  ...
  ttl: {
    ...
    mappls: 6 * 60 * 60 * 1000, // POI directory: changes slowly, keep well inside the free quota
    bhuvan: 60 * 60 * 1000,
    jctslPtal: 24 * 60 * 60 * 1000, // static import, rebuilt by scripts/fetch-jctsl-ptal.js
    worldpop: 24 * 60 * 60 * 1000, // static import, rebuilt by scripts/fetch-worldpop.js
    earthEngine: 6 * 60 * 60 * 1000, // MODIS composites update daily at most
  },
