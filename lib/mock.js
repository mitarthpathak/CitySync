...
const { validateEvent, withDefaults } = require('./event');
...
function tagAsMock(event) {
  ...
  return {
    ...
    isSimulated: true,
    layer: event.layer, // withDefaults() below fills the right default per type if this is absent
    tag: 'SIMULATED',
...
async function loadMockEvents() {
  ...
  for (const raw of parsed) {
    const event = withDefaults(tagAsMock(raw));
