...
const SOURCES = [
  ...
  { key: 'mappls', label: 'Mappls nearest emergency facilities (Amer + Achrol)', layer: 'pois', category: 'Geocoding', types: [], run: fetchMappls, ttlMs: config.ttl.mappls, stats: getMapplsStats, enabled: Boolean(config.keys.mapplsApiKey), reason: 'disabled: MAPPLS_API_KEY not set', requiresKey: true, keyConfigured: Boolean(config.keys.mapplsApiKey), attribution: 'Mappls (MapmyIndia)', docsUrl: 'https://developer.mappls.com/mapping/nearby-api' },
  { key: 'bhuvan', label: 'ISRO Bhuvan road routes (Amer + Achrol to critical destinations)', layer: 'access_routes', category: 'Routing', types: [], run: fetchBhuvan, ttlMs: config.ttl.bhuvan, retryMs: 10 * 60 * 1000 /* slow, flaky server */, stats: getBhuvanStats, enabled: Boolean(config.keys.bhuvanKey), reason: 'disabled: BHUVAN_KEY not set', requiresKey: true, keyConfigured: Boolean(config.keys.bhuvanKey), attribution: 'ISRO Bhuvan (NRSC)', docsUrl: 'https://bhuvan-app1.nrsc.gov.in/api/' },
