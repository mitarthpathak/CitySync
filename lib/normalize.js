...
/**
 * GET /api/sources shape: one row per source (live AND planned/disabled), judge-readable.
 * `keyConfigured` is a boolean ONLY - never the key value itself.
 */
async function getSourcesApiReport() {
  const { events } = await collect();
  return SOURCES.map((src) => {
    ...
    return {
      ...
      status: s.status,
      requiresKey: Boolean(src.requiresKey),
      keyConfigured: src.requiresKey ? Boolean(src.keyConfigured) : true,
      lastFetch: s.lastSuccess ?? s.lastAttempt ?? null,
      eventCount: src.onRequest ? events.filter((e) => e.layer === src.layer).length : s.events.length,
      attribution: src.attribution ?? null,
      docsUrl: src.docsUrl ?? null,
      ...(src.reason ? { reason: src.reason } : {}),
    };
  });
}

module.exports = { getEvents, getSnapshot, getSourceReport, getSourcesApiReport, applyScope, applyLayers, startScheduler, stopScheduler, refresh };
