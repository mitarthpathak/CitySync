...
const { fetchGdelt } = require('../adapters/gdelt');
...
const FEEDS = [
  ...
  { key: 'waterlogging', types: ['waterlogging'], run: fetchWaterlogging, cache: true, okStatus: 'live' },
  { key: 'waqi', types: ['air_quality'], run: fetchWaqi, cache: true, okStatus: 'live' },
  { key: 'gdelt', types: ['news'], run: fetchGdelt, cache: true, okStatus: 'live' },
  { key: 'exposure', types: ['exposure'], run: fetchExposure, cache: true, okStatus: 'live' },
  { key: 'historical', types: ['weather'], run: fetchHistorical, cache: true, okStatus: 'live' },
  { key: 'tomtom', types: ['traffic'], run: fetchTomTom, cache: true, okStatus: 'live' },
  { key: 'context', types: ['event'], run: fetchContext, cache: true, okStatus: 'live' },
];
...
async function attempt(feed) {
  try {
    ...
    if (!err.disabled) console.warn(`[${feed.key}] feed failed (${err.message || err}) - falling back to mock`);
    throw err;
  }
}
...
async function collect() {
  ...
  async function useMock(feed) {
    ...
    feeds[feed.key] = ((feed.key === 'waqi' && !process.env.WAQI_TOKEN) || (feed.key === 'tomtom' && !process.env.TOMTOM_KEY)) ? 'disabled' : (slice.length ? 'mock' : 'down');
    parts.push(slice);
  }
